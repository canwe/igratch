...
process_uploaded_file(UserId, FeedId, Data, File) ->
  case check_type_file(File) of
    true ->
      {ok, Type} = mime_type:identify(File),
      Size = length(Data),%filelib:file_size(File),
      {ok, Limit}    = kvs:get(config, "storage/upload_limit",     30 * 1024 * 1024),
      {ok, NumLimit} = kvs:get(config, "storage/upload_num_limit", 15),
      NumUploads = get_num_uploads(UserId),

      Res = if  Size > Limit -> {error, "File should be less than "++ integer_to_list(Limit) ++ " bytes"};
                NumUploads >= NumLimit -> {error, "You can't upload more than "++ integer_to_list(NumLimit) ++ " files per day"};
                true ->
                  case Type of
                    "image/jpeg" -> save_image(UserId, FeedId, Data, File);
                    "image/png" -> save_image(UserId, FeedId, Data, File);
                    "image/gif" -> save_image(UserId, FeedId, Data, File);
                    "application/pdf" -> save_pdf(UserId, FeedId, Data, File);
                    "audio/mpeg" -> save_audio(UserId, FeedId, Data, File);
                    "video/x-msvideo" -> save_video(UserId, FeedId, Data, File);
                    "video/x-flv" -> save_video(UserId, FeedId, Data, File);
                    "video/mp4" -> save_video(UserId, FeedId, Data, File);
                    _OtherMime -> save_other(UserId, FeedId, Data, File)
                  end
      end,
    ...
    false -> {error, "This file type is not supported"}
  end.

check_type_file(File) ->
  {ok, FileType} = mime_type:identify(File),
  error_logger:info_msg({"FEED ATTACHMENT: CTF:", FileType}),
  Default = [%% images
               "image/jpeg",
               "image/png",
               "image/gif",
  ...
  {ok, SupportedFileType} = kvs:get(config,"webs/upload/supported_type", Default),
...
get_num_uploads(UserId) -> get_num_uploads(UserId, now_date()).

get_num_uploads(UserId, Date) -> case kvs:get(uploads, {UserId, Date}) of {ok, #uploads{counter = Counter}} -> Counter; {error, _E} -> 0 end.

increment_num_uploads(UserId) -> increment_num_uploads(UserId, now_date()).

increment_num_uploads(UserId, Date) ->
    Counter = get_num_uploads(UserId),
    % mqs:notify(["db", "user", UserId, "put"], #uploads{key={UserId, Date}, counter=Counter + 1}).
    kvs:put(#uploads{key={UserId, Date, counter=Counter+1}}).
